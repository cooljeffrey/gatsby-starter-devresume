$(function () {
  ...
  QUnit.test(
    "should close dropdown and set focus back to toggle when escape is pressed while focused on a dropdown item",
    function (assert) {
      assert.expect(3);
      var done = assert.async();
      ...
      $dropdown.parent(".dropdown").on("shown.bs.dropdown", function () {
        ...
        $item[0].focus();
        assert.ok(
          $(document.activeElement)[0] === $item[0],
          "menu item initial focus set"
        );

        // Key escape
        $item.trigger("focus").trigger(
          $.Event("keydown", {
            which: 27,
          })
        );
        ...
        assert.ok(
          !$dropdown.parent(".dropdown").hasClass("show"),
          "dropdown menu was closed after escape"
        );
        assert.ok(
          $(document.activeElement)[0] === $toggle[0],
          "toggle has focus again once menu was closed after escape"
        );
        done();
      });
  ...
  QUnit.test(
    "should allow to pass config to popper.js with `popperConfig`",
    function (assert) {
      assert.expect(1);
  ...
  QUnit.test("should destroy old popper references on toggle", function (
    assert
  ) {
    assert.expect(3);
    var done = assert.async();
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      $dropdownEl.one("shown.bs.dropdown", function () {
        ...
        assert.strictEqual(
          $(document.activeElement).hasClass("d-none"),
          false,
          ".d-none not focused"
        );
        assert.strictEqual(
