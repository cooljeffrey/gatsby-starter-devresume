$(function () {
  ...
  QUnit.test(
    "should not toggle aria-pressed on buttons with disabled class",
    function (assert) {
      assert.expect(2);
      var $btn = $(
        '<button class="btn disabled" data-toggle="button" aria-pressed="false">redux</button>'
      );
      assert.strictEqual(
        $btn.attr("aria-pressed"),
        "false",
      ...
      assert.strictEqual(
        $btn.attr("aria-pressed"),
        "false",
        "btn aria-pressed state is still false"
      );
    }
  );

  QUnit.test(
    "should not toggle aria-pressed on buttons that are disabled",
    function (assert) {
      assert.expect(2);
      var $btn = $(
        '<button class="btn" data-toggle="button" aria-pressed="false" disabled>redux</button>'
      );
      assert.strictEqual(
        $btn.attr("aria-pressed"),
        "false",
  ...
  QUnit.test(
    'should assign active class on page load to buttons with aria-pressed="true"',
    function (assert) {
      assert.expect(1);
      var done = assert.async();
      var $btn = $(
        '<button class="btn" data-toggle="button" aria-pressed="true">mdo</button>'
      );
      ...
      setTimeout(function () {
        assert.ok(
          $btn.hasClass("active"),
          'button with aria-pressed="true" has been given class active'
        );
        done();
      }, 5);
    }
  ...
  QUnit.test(
    "should assign active class on page load to button checkbox with checked attribute",
    function (assert) {
      assert.expect(1);
      var done = assert.async();
      var groupHTML =
      ...
      setTimeout(function () {
        assert.ok(
          $btn.hasClass("active"),
          "checked checkbox button has been given class active"
        );
        done();
      }, 5);
    }
  ...
  QUnit.test(
    'should remove active class on page load from buttons without aria-pressed="true"',
    function (assert) {
      assert.expect(1);
      var done = assert.async();
      var $btn = $(
        '<button class="btn active" data-toggle="button" aria-pressed="false">mdo</button>'
      );
      $btn.appendTo("#qunit-fixture");
      $(window).trigger($.Event("load"));
      setTimeout(function () {
        assert.ok(
          !$btn.hasClass("active"),
          'button without aria-pressed="true" has had active class removed'
        );
        done();
      }, 5);
    }
  ...
  QUnit.test(
    "should remove active class on page load from button checkbox without checked attribute",
    function (assert) {
      assert.expect(1);
      var done = assert.async();
      var groupHTML =
      ...
      setTimeout(function () {
        assert.ok(
          !$btn.hasClass("active"),
          "unchecked checkbox button has had active class removed"
        );
        done();
      }, 5);
    }
  ...
  QUnit.test(
    "should not set active class if inner hidden checkbox is disabled but author forgot to set disabled class on outer button",
    function (assert) {
      assert.expect(4);
      var groupHTML =
        '<div class="btn-group" data-toggle="buttons">' +
  ...
  QUnit.test(
    "should correctly set checked state on input and active class on label when using <label><input></label> structure",
    function (assert) {
      assert.expect(4);
      var groupHTML =
        '<div class="btn-group" data-toggle="buttons">' +
  ...
  QUnit.test(
    "should correctly set checked state on input and active class on the faked button when using <div><input></div> structure",
    function (assert) {
      assert.expect(4);
      var groupHTML =
        '<div class="btn-group" data-toggle="buttons">' +
  ...
  QUnit.test(
    "should not do anything if the click was just sent to the outer container with data-toggle",
    function (assert) {
      assert.expect(4);
      var groupHTML =
        '<div class="btn-group" data-toggle="buttons">' +
  ...
  QUnit.test(
    'should not try and set checked property on an input of type="hidden"',
    function (assert) {
      assert.expect(2);
      var groupHTML =
        '<div class="btn-group" data-toggle="buttons">' +
      ...
      assert.ok(
        !$input.prop("checked"),
        "hidden input initially has no checked property"
      );
      $label[0].click(); // fire a real click on the DOM node itself, not a click() on the jQuery object that just aliases to trigger('click')
      assert.ok(
        !$input.prop("checked"),
        "hidden input does not have a checked property"
      );
    }
  );

  QUnit.test(
    "should not try and set checked property on an input that is not a radio button or checkbox",
    function (assert) {
      assert.expect(2);
      var groupHTML =
        '<div class="btn-group" data-toggle="buttons">' +
      ...
      assert.ok(
        !$input.prop("checked"),
        "text input initially has no checked property"
      );
      $label[0].click(); // fire a real click on the DOM node itself, not a click() on the jQuery object that just aliases to trigger('click')
      assert.ok(
        !$input.prop("checked"),
        "text input does not have a checked property"
      );
    }
  );
